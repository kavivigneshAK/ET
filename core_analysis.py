...
async def generate_forensic_report(file_path: str) -> str:
    """Uploads the local document to Gemini and extracts the forensic report asynchronously."""
    # Dynamically hot-reload environment configurations directly from .env.example or .env
    ...
    # Wait for the file to be processed by Google API
    ...
    # Falling back to 'gemini-2.5-flash' because the 'pro' tier has extremely tight rate limits 
    # on free accounts (often throwing 429 Quota Exceeded limits). Flash has 15x higher limits!
    model = genai.GenerativeModel('gemini-2.5-flash')
    
    try:
        # Request generation with a large timeout to handle massive RHP PDFs
        ...
        if "404" in err_msg or "not found" in err_msg:
            raise ValueError(f"Model Not Found: Your API tier may lack access to Gemini-1.5-Flash (Details: {e})")
    ...
    # Cleanup the file from Gemini's servers immediately after successful usage
    try:
        genai.delete_file(pdf_file.name)
    except Exception:
        pass
